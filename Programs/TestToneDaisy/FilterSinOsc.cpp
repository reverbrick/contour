...
class mydsp : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("filename", "FilterSinOsc.dsp");
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		float fSlow1 = std::exp((0.0f - (fConst2 / float(fHslider2))));
		float fSlow2 = (440.0f * (std::pow(2.0f, (0.0833333358f * (float(fHslider1) + -49.0f))) * (1.0f - fSlow1)));
		for (int i = 0; (i < count); i = (i + 1)) {
			...
			fRec2[0] = ((float((1 - iVec0[1])) + (fRec2[1] * fTemp2)) - (fTemp1 * fRec1[1]));
...
FilterSinOsc::FilterSinOsc(int sample_rate, int buffer_size)
{
    fBS = buffer_size;
    fDSP = new mydsp();
...
void FilterSinOsc::setParamValue(const std::string& path, float value)
{
    fUI->setParamValue(path, value);
}

void FilterSinOsc::configureI2S(int sample_rate, int buffer_size, i2s_pin_config_t pin_config)
{
    #if A1S_BOARD
...
void FilterSinOsc::audioTaskHandler(void* arg)
{
    FilterSinOsc* audio = (FilterSinOsc*)arg;
    audio->audioTask();
}
